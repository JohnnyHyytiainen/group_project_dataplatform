...
Establishing a connection to a PostgreSQL database over a network requires an asynchronous 'handshake'. If 100 users (or a dashboard) call our API at the same time and create 100 new handshakes, the database will crash. As Kristoffer would note, that is _not_ industry standard.
...
Our API is built with a _Defense in Depth_ mindset to protect our data and ensure high technical quality.
